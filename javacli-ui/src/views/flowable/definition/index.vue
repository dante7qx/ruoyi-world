<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" fit :data="definitionList" border @selection-change="handleSelectionChange" v-adaptive height="100">
  ...
  methods: {
    ...
    getList() {
      ...
      if (null != this.daterangeDeployTime && '' != this.daterangeDeployTime) {
        this.queryParams.params["beginDeploymentTime"] = this.daterangeDeployTime[0] + ' 00:00:00';
        this.queryParams.params["endDeploymentTime"] = this.daterangeDeployTime[1] + ' 23:59:59';
      }
    ...
    /** 跳转到流程设计页面 */
    handleLoadXml(row){
      const deployId = row != null ? row.deploymentId : null;
      this.$router.push({ path: '/flowable/definition/model', query: { deployId: deployId }})
    },
    /** 流程图查看 */
