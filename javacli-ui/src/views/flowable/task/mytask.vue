<template>
  <div class="app-container">
    ...
    <el-table v-loading="loading" :data="flowList" v-adaptive height="100">
      ...
      <el-table-column label="操作" align="center" class-name="small-padding fixed-width" width="200">
        <template slot-scope="scope">
          ...
          <el-button
            ...
            v-if="scope.row.recordCount == 2"
            @click="handleRevoke(scope.row.bizUid)"
          >撤销</el-button>
          <el-button
            size="mini"
            type="text"
            icon="el-icon-delete"
            v-if="scope.row.recordCount == 0 || (scope.row.recordCount > 2 && !scope.row.flowResult && scope.row.firstTask)" 
    ...
    <!-- 新建流程 -->
    <el-dialog title="新建流程" :visible.sync="openNewFlow" width="850px" v-dialog-drag append-to-body>
      <new-flow v-if="openNewFlow" @closeWindow="closeNewFlowWin" />
    </el-dialog>
...
<script>
import { myList, revokeFlow, delFlow } from "@/api/flowable/process";
import NewFlow from "@/views/flowable/task/newflow";
...
export default {
  ...
  methods: {
    ...
    closeNewFlowWin(flowType) {
      this.openNewFlow = false
      if(flowType) {
        this.key = flowType
        this.flowType = flowType
        this.pageCtl = 1
        this.title = `新建${this.selectDictLabel(this.dict.type.sys_process_type, flowType)}`
        this.open = true;
      }
    },
    closeFlowWin() {
