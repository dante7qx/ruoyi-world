	...
	protected void handleDataScope(final JoinPoint joinPoint, DataScope controllerDataScope) {
		...
		if (ObjectUtil.isNotNull(loginUser)) {
			...
			if (ObjectUtil.isNotNull(currentUser) && !currentUser.isAdmin()) {
				String permission = StrUtil.emptyToDefault(controllerDataScope.permission(),
	...
	public static void dataScopeFilter(JoinPoint joinPoint, SysUser user, String deptAlias, String userAlias,
		...
		for (SysRole role : user.getRoles()) {
			...
			if (StrUtil.isNotEmpty(permission) && CollUtil.isNotEmpty(role.getPermissions())
					&& !CollUtil.containsAny(role.getPermissions(), ListUtil.of(Convert.toStrArray(permission)))) {
				continue;
			}
			if (DATA_SCOPE_ALL.equals(dataScope)) {
				...
				sqlString.append(
						StrUtil.format(" OR {}.dept_id IN ( SELECT dept_id FROM sys_role_dept WHERE role_id = {} ) ",
