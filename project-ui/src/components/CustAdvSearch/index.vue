          ...
          <el-tabs type="border-card" v-model="activeTab" @tab-click="handleTabClick">
            <el-tab-pane label="模版条件" name="cond" lazy>
              <el-table ref="condTable" :data="columns" row-key="colName" :max-height="tableHeight" v-if="templateId > 0">
                ...
                <el-table-column label="查询" min-width="3%" align="center">
                  <template slot-scope="scope">
      ...
      if(data.queryId > 0) {
        ...
        <span>
          ...
          <span v-show="data.queryId > 0" style="margin-left: 20px;">
            <el-tooltip effect="dark" content="编辑" placement="right">
              <i class="el-icon-edit" on-click={ () => this.editTemplate(node) }></i>
            </el-tooltip>
            <el-tooltip effect="dark" content="删除" placement="right">
              <i class="el-icon-delete" on-click={ () => this.removeTemplate(data.queryId) } style="margin-left: 10px;"></i>
            </el-tooltip>
          </span>
        </span>);
      } else {
        return (
          <span>
            <span on-click={ () => this.nodeClick(data.queryId) }>{node.label}</span>
            <span v-show="data.queryId > 0" style="margin-left: 20px;">
              <el-tooltip effect="dark" content="新增模板" placement="right">
                <i class="el-icon-plus" on-click={ () => this.addTemplate() }></i>
              </el-tooltip>
            </span>
          </span>);
      }
