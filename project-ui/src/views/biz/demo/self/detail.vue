...
<script>
import { getDemoself, addDemoself, updateDemoself } from "@/api/biz/demo/demoself";
...
export default {
  ...
  methods: {
    loadForm() {
      this.reset();
      if(this.demoId > 0) {
        getDemoself(this.demoId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.demoId != null) {
            updateDemoself(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.cancel();
            });
          } else {
            addDemoself(this.form).then(response => {
